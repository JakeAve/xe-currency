...
const Home = (): JSX.Element => {
  ...
  const { toggleOpen } = dialogControls;
  ...
  return (
    <>
      <nav>
        <ul>
          <li>
            <h1 className="home-link">
              <Link to="/">{translatedStrings.title}</Link>
            </h1>
          </li>
          <li>
        ...
        <button className="lang-btn" aria-label={translatedStrings.langButton} onClick={toggleOpen}>
          <svg
            aria-hidden="true"
            focusable="false"
            data-prefix="fas"
      ...
      <LanguageDialog dialogControls={dialogControls} />
    </>
  );
};
